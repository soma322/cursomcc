...
public class TiendaTarjeta extends Tienda implements Pago{
	boolean efectivo = false;
	boolean tarjeta = true;
	double saldo;
	
	
	public TiendaTarjeta(String nombre, String direccion, int telefono) {
	...
	public void realizarPago(double monto,String tipoPago) {
		if(tipoPago != "tarjeta") {
			System.out.println("Tienda solo aceptar tarjeta");
		}else {
			if(saldo == 0) {
	...
	public void realizarPago(double cantidad) {
		System.out.println("Se ha realizado un pago de $" + cantidad + " en la tienda solo tarjeta");
